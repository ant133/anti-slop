...
> By default, ask the user when antislop applies: during the work, or after it is done. A saved `during` or `after` preference skips only this question and must be announced when antislop activates.
...
When an explicit session instruction or a saved `during` or `after` preference resolves the mode, announce it once in the user's chat language before applying the skill: **"antislop active: during (global preference)."** Use `after` as appropriate and `session override` when that is the source. When the default question resolves the mode, proceed as before without adding a second notice. Additional antislop skills reuse this mode without another question or notice. Announce again only if the mode changes.
